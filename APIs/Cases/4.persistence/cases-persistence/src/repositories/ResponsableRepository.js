...
const IResponsableRepository = require("../Interface/IResponsableRepository");
const writable = ["nombre","telefono","cargo","correoEmail"];
class ResponsableRepository extends IResponsableRepository {
  constructor(model) {
    super();
    this.model = model;
  }
  ...
  async requireRecord(id) {
    ...
    if (!record) throw new NotFoundError("Responsable no encontrado");
    return record;
  }
